...
def main():
    ...
    for agency in allAgencies:
        ...
        email = agency['email']
        if len(email) > 0:
            matches = phAgencies.find({ "$and": [{"email": email},{"_id":{'$ne':_id}}]})
            matching_ids = [str(some_agency["_id"]) for some_agency in matches]
            phAgencies.update({"_id":_id}, {"$set": {"sharedEmailAgencies": list(matching_ids)}})
        else:
            phAgencies.update({"_id":_id}, {"$set": {"sharedEmailAgencies": []}})
        count += 1
        if count % 20 == 0:
            print count
