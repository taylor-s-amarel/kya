...
def print_agency_agency_telephone(text_file, collectionName):
    ...
    for i, agency in enumerate(allAgencies):
        print(i)
        similarAgencies = agency["sharedPhoneAgencies"]
        if(len(similarAgencies) == 0):
            continue
        main_line = agency["_id"] + "|" + agency["english_name"] + "|" + agency["telephone"] + "\n"
        text_file.write(main_line)
        for similarAgencyId in similarAgencies:
            similarAgency = hkAgencies.find_one({"_id": similarAgencyId})
            sub_line = similarAgency["_id"] + "|" + similarAgency["english_name"] + "|" + agency["telephone"] + "\n"
...
def execute(timestamp):
    ...
    path = './archive/' + collectionName + '/agency_agency_telephone.csv'
    text_file = codecs.open(path, "a", encoding="utf-8")
    text_file.write(u"govid|english_name|telephone" + "\n")
    text_file.write("\n")
    print_agency_agency_telephone(text_file, collectionName)
