...
def print_agency_agency_similar_address(text_file, collectionName):
    ...
    for i, agency in enumerate(allAgencies):
        print(i)
        similarAgencies = agency["addressTextSimilarityAgencies"]
...
def execute(timestamp):
    ...
    path = './archive/' + collectionName + '/agency_agency_similar_address.csv'
    text_file = codecs.open(path, "a", encoding="utf-8")
    text_file.write(u"govid|english_name|address" + "\n")
    text_file.write("\n")
    print_agency_agency_similar_address(text_file, collectionName)
